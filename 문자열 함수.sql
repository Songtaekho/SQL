...
--TRIM - 양쪽공백제거, LTRIM - 왼쪽에서제거, RTRIM - 오른쪽에서제거
SELECT TRIM(' HELLO WORLD '), LTRIM( ' HELLO WORLD ' ), RTRIM( ' HELLO WORLD ') FROM DUAL;

SELECT LTRIM( 'HELLO WOLRD', 'HELLO') FROM DUAL;

--REPLACE(타겟, 찾을문자, 변경문자)
SELECT REPLACE('피카츄 라이츄 파이리 꼬부기 버터플', '꼬부기', '어니부기') FROM DUAL;

--함수는 NESTED(중첩) 이 가능하다.
SELECT REPLACE(REPLACE('피카츄 라이츄 파이리 꼬부기 버터플', '꼬부기', '어니부기'),' ' ,' > ') FROM DUAL;
...
SELECT CONCAT ( CONCAT( FIRST_NAME, ' '), LAST_NAME) AS 이름  
    ...
    FROM EMPLOYEES ORDER BY 이름;  -- FIRST_NAME ||' '|| LAST_NAME AS 이름,  REPLACE (HIRE_DATE, '/', '') AS 입사일자
