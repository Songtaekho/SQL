...
--직원 번호가 103번인 사람과 동일한 직무를 가진 사람
...
--스칼라 쿼리는 한번에 하나의 컬럼을 가지고 옵니다. 많은 일을 가지고 올대는 가독성이 떨어질 수 있습니다.
SELECT FIRST_NAME,
        (SELECT DEPARTMENT_NAME FROM DEPARTMENTS D WHERE D.DEPARTMENT_ID = E.DEPARTMENT_ID),
...
--ROWNUM은 조회된 순서에 대해서 번호가 붙기대문에 ORDER BY를 시키면 순서가 뒤바뀝니다.
SELECT ROWNUM,
        EMPLOYEE_ID,
        FIRST_NAME,
        SALARY
...
WHERE ROWNUM > 10 AND ROWNUM <= 20; -- 10~20번쨰 데이터가 나와야 하는데, ROWNUM은 1부터만 조회가 가능합니다.
