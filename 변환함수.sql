...
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') FROM DUAL; -- 데이트 포멧형식이 아닌값은 ""로 묶으면 됩니다.
...
SELECT TO_CHAR(20000, '9999') FROM DUAL; -- 자리가 부족하면 # 으로 처리함
SELECT TO_CHAR(20000.123, '99999.999') FROM DUAL;
SELECT TO_CHAR(20000, '$999,999.99') FROM DUAL; -- $는 그냥 쓰면 됩니다. L을 붙이면 지역화폐
...
SELECT FIRST_NAME, SALARY, COMMISSION_PCT, SALARY + NVL (SALARY * commission_pct, 0) AS 실제급여 FROM EMPLOYEES;
